    ...
    <div className="flex flex-col h-full space-y-10">
      ...
      <div className="bg-white rounded-[2rem] shadow-lg border border-slate-100 p-6 space-y-4">
          <div className="flex items-center justify-between px-2">
              <h3 className="text-[10px] md:text-xs font-black text-slate-900 uppercase tracking-[0.3em]">Tactical Standings</h3>
              <span className="text-[9px] font-black text-indigo-500 bg-indigo-50 px-3 py-1 rounded-lg uppercase">{participants.length} OPS</span>
          </div>
              ...
              <table className="w-full text-left border-collapse min-w-[300px]">
                  <thead className="bg-slate-50/50">
                      <tr>
                          ...
                          <th className="px-3 py-3 text-[10px] font-black text-slate-400 border-b border-slate-100 uppercase">Operator</th>
                          <th className="px-3 py-3 text-[10px] font-black text-slate-400 border-b border-slate-100 uppercase text-right w-20">Score</th>
                    ...
                    <AnimatePresence mode="popLayout">
                       ...
                       {sortedSquad.length === 0 && (
                           <motion.tr initial={{ opacity: 0 }} animate={{ opacity: 1 }} key="loading">
                             <td colSpan="4" className="py-20 text-center text-sm font-black text-slate-300 uppercase tracking-[0.5em] animate-pulse">Scanning squadron signal...</td>
                           </motion.tr>
                       )}
                    </AnimatePresence>
                  </tbody>
      ...
      {session?.role === 'HOST' && (
          <div className="bg-slate-900 rounded-2xl p-5 border-t-4 border-indigo-600 space-y-4 shadow-lg">
               <div className="flex items-center gap-1.5">
                  ...
                  <h3 className="text-[10px] font-black text-indigo-400 uppercase tracking-[0.3em]">Commander Broadcast</h3>
               </div>
      ...
      {/* 💬 COMBAT COMMS CHAT */}
      <div className="flex-1 min-h-[400px] md:min-h-[550px] flex flex-col bg-[#050816] rounded-[2rem] md:rounded-[3.5rem] shadow-2xl border-4 border-slate-900 overflow-hidden relative">
          <div className="px-6 md:px-8 py-5 md:py-7 border-b border-white/5 flex items-center justify-between bg-black/40 backdrop-blur-md">
              <div className="flex items-center gap-4">
                  <div className="w-2.5 h-2.5 rounded-full bg-green-500 shadow-[0_0_15px_rgba(34,197,94,0.8)] animate-pulse"></div>
          ...
          <div className="flex-1 overflow-y-auto p-8 space-y-6 custom-scrollbar">
              {chatMessages.length === 0 && (
                  <div className="flex flex-col items-center justify-center h-full opacity-40 text-center space-y-6">
                      ...
                      <p className="text-sm font-black text-slate-300 uppercase tracking-[0.5em]">Secure Uplink established.</p>
                  </div>
              )}
              {chatMessages.map((m) => (
