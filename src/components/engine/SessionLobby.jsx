...
export default function SessionLobby({ sessionId, isHost, onApproveGuest, onDenyGuest, sessionDuration = 0, onLeave }) {
  ...
  useEffect(() => {
    if (participants.length < prevParticipantsCount.current) {
        toast('A participant has left the room.', { icon: '👋', duration: 3000 });
    }
      ...
      return (
          <div className="w-full max-w-6xl bg-[var(--bg-secondary)] rounded-2xl shadow-2xl p-6 sm:p-12 text-center space-y-8 border border-[var(--card-border)]">
               ...
               <div className="space-y-6">
                  <h2 className="text-3xl sm:text-5xl font-black text-slate-900 uppercase tracking-tighter">Clearance Pending</h2>
                  <p className="text-base sm:text-lg font-bold text-slate-500 max-w-lg mx-auto leading-relaxed">
                     Secure connection established. Standing by for **Host {sessionId}** to authorize your entry.
                  </p>
               </div>
               <div className="pt-4">
                    <button 
      ...
      <div className="w-full bg-slate-900 text-white px-5 py-4 sm:px-8 sm:py-4 rounded-2xl flex flex-col sm:flex-row items-center justify-between gap-4 sm:gap-6 border border-slate-800 shadow-xl">
          <div className="flex items-center justify-between w-full sm:w-auto gap-6 sm:gap-10">
              <div className="flex items-center gap-3">
                ...
                <span className="text-[10px] sm:text-[11px] font-black uppercase tracking-[0.2em] sm:tracking-[0.3em] text-green-400 whitespace-nowrap">Live Quiz Room</span>
              </div>
              <div className="hidden sm:block h-4 w-px bg-slate-700"></div>
              <div className="flex items-center gap-4 sm:gap-6">
                  <div className="flex items-center gap-2">
                    <span className="text-[9px] sm:text-[10px] font-black text-slate-500 uppercase tracking-widest">Uptime</span>
                    <span className="text-[10px] sm:text-[11px] font-black text-white tabular-nums">{formatDuration(sessionDuration)}</span>
                  </div>
                  <div className="flex items-center gap-2">
                    <span className="text-[9px] sm:text-[10px] font-black text-slate-500 uppercase tracking-widest">Players</span>
          ...
          {isHost && (
              <div className="flex items-center gap-3 w-full sm:w-auto justify-end pt-3 sm:pt-0 border-t border-slate-800 sm:border-t-0">
                  <button 
                    onClick={() => { socketService.getSocket()?.emit('FORCE_ROOM_SYNC', { sessionId }); toast.success('Squad Sync Complete!'); }} 
                  ...
                  >
                    <span>🔄</span>
                    <span className="sm:hidden text-[10px] font-black uppercase tracking-widest">Sync Room</span>
                  </button>
                  <button 
                    onClick={() => { if(window.confirm('Terminate Live Quiz Session?')) sendAction('TERMINATE', { status: 'LOBBY' }); }}
                  ...
                  >
                    <span>🛑</span>
                    <span className="sm:hidden text-[10px] font-black uppercase tracking-widest">Terminal</span>
                  </button>
              </div>
          )}
      </div>
          ...
          <div ref={hubRef} className="lg:col-span-8 flex flex-col gap-8 order-1">
            ...
            <div className="bg-white rounded-[2rem] border border-slate-100 shadow-xl p-8 sm:p-10 space-y-8 relative overflow-hidden group">
                ...
                <div className="space-y-1 relative">
                    <h3 className="text-2xl sm:text-3xl font-black text-slate-900 tracking-tighter uppercase">Invite Players</h3>
                    <p className="text-[10px] sm:text-xs font-black text-indigo-500 uppercase tracking-[0.2em] leading-none">MULTIPLY YOUR SQUAD</p>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-12 gap-8 items-center">
                    <div className="md:col-span-7 space-y-4">
                        <div className="flex bg-slate-50/50 p-1.5 rounded-2xl border border-slate-100 focus-within:border-indigo-300 transition-all shadow-inner">
                            ...
                            <button 
                                onClick={() => {
                                    ...
                                    if (navigator.share) {
                                        setTimeout(() => { navigator.share({ title: 'Join my Live Quiz!', url: inviteUrl }).catch(() => {}); }, 100);
                                    }
                                }}
                        ...
                        <p className="hidden sm:block text-[11px] font-bold text-slate-400 leading-relaxed">Share this link directly or use the recruitment button to invite friends to your live quiz operation.</p>
                    </div>
                    <div className="md:col-span-5 space-y-3">
                        <a 
                            href={`https://wa.me/?text=${encodeURIComponent('Join my Live Quiz room and compete with me! 🚀 ' + inviteUrl)}`}
                            target="_blank"
                        ...
                        >
                            ...
                            <span className="font-black text-xs uppercase tracking-widest">Invite on WhatsApp</span>
                        </a>
                        <p className="text-[10px] font-bold text-green-600 text-center uppercase tracking-tighter">Invite your friends on WhatsApp and play amazing live quiz with them!</p>
                    </div>
                </div>
                ) : (
                <div className="py-12 bg-slate-50 border-2 border-dashed border-slate-200 rounded-3xl animate-pulse flex items-center justify-center text-xs font-black text-slate-300 uppercase tracking-widest">Synchronizing Encryption...</div>
                )}
            </div>
               ...
               <div ref={selectionRef} className="flex-1 flex flex-col gap-8">
                        ...
                        <div className={`space-y-6 sm:space-y-8 ${selectedCategory ? 'opacity-40 hover:opacity-100 transition-all duration-500' : ''}`}>
                            <div className="flex items-center justify-between px-2">
                                <h4 className="text-[10px] sm:text-[11px] font-black text-slate-400 uppercase tracking-[0.3em] sm:tracking-[0.4em]">
                                    {selectedCategory ? 'Alternative Intelligence' : 'Select Quiz Topic'}
                                </h4>
                                {selectedCategory && (
                                    <button onClick={() => { setSelectedCategory(null); setNavStack([]); setCurrentParentId(null); }} className="text-[10px] font-black text-indigo-500 uppercase hover:text-indigo-400 tracking-widest border-b-2 border-indigo-500">RESET MISSION</button>
                                )}
                            </div>
                            
                            {loading && categories.length === 0 ? (
                                <div className="py-24 sm:py-32 text-center text-sm font-black text-slate-300 uppercase animate-pulse tracking-[0.3em] sm:tracking-[0.5em]">Scanning Knowledge Grid...</div>
                            ) : (
                                    ...
                                    {filteredCategories.map(c => (
                                        ...
                                        >
                                            ...
                                            <div className="text-left flex-1 min-w-0">
                                                ...
                                                <p className={`text-[9px] sm:text-[10px] font-bold opacity-60 tracking-widest uppercase truncate ${selectedCategory?.id === c.id ? 'text-indigo-200' : 'text-slate-400'}`}>{c.questionCount} ASSESSMENTS</p>
                                            </div>
                                        </button>
                                    ))}
                                </div>
                ...
                <div className="flex flex-col items-center justify-center h-full text-center space-y-12 sm:space-y-16 py-16 sm:py-28 px-6 sm:px-10">
                    ...
                    <div className="space-y-6 max-w-lg relative">
                        <h3 className="text-3xl sm:text-5xl font-black text-slate-900 uppercase tracking-tighter leading-none">Ready for Action!</h3>
                        <p className="text-[10px] sm:text-sm font-black text-indigo-500 uppercase tracking-[0.5em] animate-pulse">Transmission Established</p>
                        <p className="text-sm sm:text-base font-bold text-slate-500 leading-relaxed uppercase px-4 sm:px-8">Connected to secure operational lounge. Standing by for **Quiz Host {session?.hostName || ''}** to authorize session parameters.</p>
                    </div>
              ...
              <div className="bg-white rounded-[2rem] border border-slate-100 shadow-xl overflow-hidden">
                  <div className="p-8 sm:p-10 border-b border-slate-50 flex items-center justify-between bg-indigo-50/20">
                     <div className="space-y-1">
                        <h3 className="text-xl sm:text-2xl font-black text-slate-900 tracking-tighter uppercase">Active Players</h3>
                        <p className="text-[10px] sm:text-[11px] font-bold text-slate-400 uppercase tracking-widest">{participants.length} Operatives Ready</p>
                     </div>
                  ...
                  <div className="p-6 sm:p-8 space-y-4 max-h-[400px] lg:max-h-[600px] overflow-y-auto custom-scrollbar">
                      <div className="flex items-center justify-between p-4 sm:p-5 bg-indigo-600 rounded-2xl border border-indigo-500 relative group text-white shadow-xl shadow-indigo-100">
                         <div className="absolute top-0 right-3 px-3 py-1 bg-white text-indigo-600 text-[8px] font-black uppercase tracking-widest rounded-b-md shadow-sm">Host 👑</div>
                         <div className="flex items-center gap-3 sm:gap-4">
                            <div className="relative">
                               <div className="w-10 h-10 sm:w-12 sm:h-12 bg-white/20 backdrop-blur-md rounded-xl flex items-center justify-center text-white text-xl sm:text-2xl font-black italic shadow-inner">
                                   {isHost ? 'Y' : (hostMember?.userName?.[0] || 'L')}
                               </div>
                      ...
                      {squadMembers.map(p => {
                        ...
                        return (
                          <div key={p.userId} className="flex items-center justify-between p-4 sm:p-5 bg-slate-50 border border-slate-100 rounded-2xl shadow-sm hover:border-indigo-400 hover:bg-white group transition-all">
                             <div className="flex items-center gap-3 sm:gap-4">
                                 <div className="relative">
                                    <div className="w-10 h-10 sm:w-11 sm:h-11 bg-white border border-slate-200 rounded-xl flex items-center justify-center text-[10px] sm:text-[11px] font-black uppercase text-slate-400 group-hover:text-indigo-600 group-hover:border-indigo-100 transition-all shadow-sm">{p.userName?.[0]}</div>
                                 ...
                                 <div className="flex flex-col">
                                    ...
                                    <span className="text-[8px] sm:text-[9px] font-black uppercase tracking-widest text-slate-400 mt-1">{isOffline ? 'Left 💀' : isIdle ? 'Idle' : 'Active'}</span>
                                 </div>
                             </div>
                             {isHost && (
                                 <button onClick={() => sendAction('KICK_PARTICIPANT', { userId: p.userId })} className="w-9 h-9 sm:w-10 sm:h-10 flex items-center justify-center bg-white text-red-400 hover:bg-red-500 hover:text-white rounded-xl transition-all shadow-sm opacity-0 group-hover:opacity-100" title="Dismiss Operative">×</button>
                             )}
                          </div>
                        );
                      })}
                      ...
                      {squadMembers.length === 0 && (
                          <div className="py-16 sm:py-20 px-8 text-center space-y-6 opacity-40 grayscale animate-pulse">
                               ...
                               <div className="space-y-2">
                                  <p className="text-[9px] sm:text-[10px] font-black uppercase tracking-[0.4em] sm:tracking-[0.5em] text-slate-400">Awaiting Deployment...</p>
